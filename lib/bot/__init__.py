...
class Bot(BotBase):
  ...
  async def print_message(self):
    self.stdout = self.get_channel(600001330920292353) or self.get_channel(
      965478504038813796)
    self.guild = self.get_guild(535693863696990208) or self.get_guild(
      955631286263885835)
    pass
...
Date: {self.dt.day:02d}/{self.dt.month:02d}/{self.dt.year}""")
  ...
  async def on_command_error(self, ctx, exc):
    if isinstance(exc, CommandNotFound):
      ...
      await ctx.send(
        "That user is not in this server, Command could not be completed.",
        delete_after=8)
      ...
      embed = Embed(title="You are missing permissions to run this command!",
                    description=f"{exc}")
      ...
      embed = Embed(title="Color Input Incorrect.", description=f"{exc}")
  ...
  async def on_ready(self):
    if not self.ready:
      ...
      while not self.cogs_ready.all_ready():
        await sleep(0.5)
      print("\nCog Setup Complete")
      self.stdout = self.stdout = self.get_channel(
        600001330920292353) or self.get_channel(965478504038813796)
      self.guild = self.get_guild(535693863696990208) or self.get_guild(
        955631286263885835)
      self.ready = True
